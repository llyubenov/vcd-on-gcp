...
module "gcve-network" {
  ...
  external_ip_enabled     = var.external_ip_enabled
}
...
module "gcve-prime-client" {
  source                   = "../../modules/gcve-prime-client"
  region                   = var.gcloud_region
  private_cloud_id         = "projects/${var.gcloud_project}/locations/${lookup(var.gcve_sddc_variables[0], "sddc_zone")}/privateClouds/${lookup(var.gcve_sddc_variables[0], "sddc_name")}"
  prime_client_internal_ip = var.prime_client_internal_ip
  gcve_network_policy_id   = module.gcve-network.gcve_network_policy_id
}
